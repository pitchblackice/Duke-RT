...
try {
    ...
    Assert-True ([double]$profile.defaultFull.workloadPopulation.voxel_occurrences.p50 -eq 136) "Voxel occurrence population summary is wrong."
    Assert-True ([double]$profile.defaultFull.workloadPopulation.voxel_instance_prims.p99 -eq 5000003) "Voxel primitive population summary is wrong."
    Assert-True ([double]$profile.defaultOmit.workloadPopulation.voxel_occurrences.max -eq 0) "Omitted voxel occurrence population is wrong."
    Assert-True ([double]$profile.defaultOmit.workloadPopulation.voxel_instance_prims.max -eq 0) "Omitted voxel primitive population is wrong."
