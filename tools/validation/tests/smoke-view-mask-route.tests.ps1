...
Assert-Match $settings 'viewRoute\s*=\s*\(uint32_t\)std::clamp\(\(int\)nri_ptsmokeviewroute,\s*0,\s*1\)' 'Only dense and masked-dense static routes may resolve.'
...
Assert-NotMatch $runtime 'CmdDispatchIndirect|available.*headroom|gpu.*budget' 'Slice 3B may not add indirect execution or adaptive policy.'
