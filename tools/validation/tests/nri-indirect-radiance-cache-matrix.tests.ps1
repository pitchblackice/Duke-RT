...
Assert-True ($runner -match 'for \(\$cycle = 1; \$cycle -le 3; \+\+\$cycle\)[\s\S]*?\$start = \(1 - \$cycle \+ \$legs\.Count\) % \$legs\.Count') 'runner must use a balanced three-cycle rotation'
...
try {
	...
	$order = @(
		@('exact', 'forced-miss', 'age-one'),
		@('age-one', 'exact', 'forced-miss'),
		@('forced-miss', 'age-one', 'exact')
	)
	$sequence = 0
	foreach ($cycleIndex in 0..2) {
		for ($ordinalIndex = 0; $ordinalIndex -lt 3; ++$ordinalIndex) {
			++$sequence
			$mode = $order[$cycleIndex][$ordinalIndex]
			$directory = Join-Path $tempRoot ("run-$sequence")
	...
	[pscustomobject]@{ schema = 1; cycles = 3; samples = 2; warmupSamples = 1; blasPolicy = 1; entries = $entries.ToArray() } |
		ConvertTo-Json -Depth 12 | Set-Content -LiteralPath $manifestPath -Encoding UTF8
	$powershell = (Get-Process -Id $PID).Path
	& $powershell -NoProfile -ExecutionPolicy Bypass -File $analyzerPath -ManifestPath $manifestPath -SummaryOutput $summaryPath | Out-Null
	Assert-True ($LASTEXITCODE -eq 0) 'valid synthetic cache matrix was rejected'
	$summary = Get-Content -LiteralPath $summaryPath -Raw | ConvertFrom-Json
	Assert-True ([bool]$summary.ok -and @($summary.runs).Count -eq 9 -and @($summary.legs).Count -eq 3) 'valid cache summary shape is wrong'
	$forced = @($summary.legs | Where-Object mode -eq 'forced-miss')[0]
	$ageOne = @($summary.legs | Where-Object mode -eq 'age-one')[0]
	...
	$forcedLog = [string](@($entries | Where-Object mode -eq 'forced-miss')[0].logPath)
	...
	Assert-True ((Invoke-AnalyzerForExitCode $powershell $analyzerPath $manifestPath $summaryPath) -ne 0) 'forced-miss accepted-hit violation was not rejected'
	...
	Assert-True ((Invoke-AnalyzerForExitCode $powershell $analyzerPath $manifestPath $summaryPath) -ne 0) 'missing exact cache endpoint was not rejected'
	Write-Utf8 -Path $forcedLog -Text $original

	$ageLog = [string](@($entries | Where-Object mode -eq 'age-one')[0].logPath)
	...
	Assert-True ((Invoke-AnalyzerForExitCode $powershell $analyzerPath $manifestPath $summaryPath) -ne 0) 'cross-leg content identity drift was not rejected'
	...
	Assert-True ((Invoke-AnalyzerForExitCode $powershell $analyzerPath $manifestPath $summaryPath) -ne 0) 'invalid GPU sample was not rejected'
}
finally {
