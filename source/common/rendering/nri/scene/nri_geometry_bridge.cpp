...
void BuildGeometry(const SceneView& sceneView, GeometryData& outGeometry)
{
	...
	{
		if (wall.vertices == nullptr || wall.vertexCount < 3)
		{
			materialIndex++;
			continue;
		}
		...
		for (uint32_t i = 1; i + 1 < wall.vertexCount; ++i)
		{
	...
	{
		if (flat.vertices == nullptr || flat.vertexCount < 3)
		{
			materialIndex++;
			continue;
		}

		for (uint32_t i = 0; i + 2 < flat.vertexCount; i += 3)
		{
