...
namespace
{
	...
	constexpr int kDukeLaserLineTile = 2567;
	constexpr float kLaserLineAxisDotTolerance = 0.995f;
	constexpr float kLaserLineNormalDotTolerance = 0.995f;
	constexpr float kLaserLinePerpendicularTolerance = 1.5f;
	constexpr float kLaserLineGapTolerance = 8.0f;

	struct LaserLineSegmentCandidate
	{
	...
	void AddScaled3(const float* base, const float* direction, float scale, float* out)
	{
		out[0] = base[0] + direction[0] * scale;
		out[1] = base[1] + direction[1] * scale;
		out[2] = base[2] + direction[2] * scale;
	}

	bool ShouldFlipCanonicalAxis(const float* axis)
	{
		if (fabsf(axis[0]) > 0.0001f)
		{
			return axis[0] < 0.0f;
		}
		if (fabsf(axis[1]) > 0.0001f)
		{
			return axis[1] < 0.0f;
		}
		return axis[2] < 0.0f;
	...
	bool SameLaserLineMaterial(const LaserLineSegmentCandidate& a, const LaserLineSegmentCandidate& b)
	{
		return a.textureId == b.textureId &&
			a.palette == b.palette &&
			a.shade == b.shade &&
			fabsf(a.alpha - b.alpha) <= 0.001f &&
			a.flags == b.flags &&
			a.drawListType == b.drawListType;
	}

	bool IsDukeLaserLineSurface(const SurfaceRef& surface)
	{
		if (surface.provenance.sourceType != SurfaceSourceType::DrawListWall ||
			surface.provenance.actorIndex < 0 ||
			(surface.material.flags & MaterialFlag_Sprite) == 0 ||
			surface.material.texture == nullptr)
		{
			return false;
		}

		const FTextureID laserLineTexid = tileGetTextureID(kDukeLaserLineTile);
		if (!laserLineTexid.isValid())
		{
			return false;
		}

		const FTextureID surfaceTexid = surface.material.texture->GetID();
		return surfaceTexid.isValid() && surfaceTexid.GetIndex() == laserLineTexid.GetIndex();
	}

	bool TryBuildLaserLineCandidate(const SurfaceRef& surface, size_t surfaceIndex, LaserLineSegmentCandidate& outCandidate)
	{
		if (!IsDukeLaserLineSurface(surface) || surface.vertices.size() != 4)
		{
			return false;
		}
		...
		float edge01[3] = {};
		float edge02[3] = {};
		Sub3(surface.vertices[1].position, surface.vertices[0].position, edge01);
		Sub3(surface.vertices[2].position, surface.vertices[0].position, edge02);

		const float edge01LengthSq = LengthSq3(edge01);
		const float edge02LengthSq = LengthSq3(edge02);
		if (edge01LengthSq <= 0.000001f || edge02LengthSq <= 0.000001f)
		{
			return false;
		}
		...
		float axis[3] = {};
		axis[0] = edge01LengthSq >= edge02LengthSq ? edge01[0] : edge02[0];
		axis[1] = edge01LengthSq >= edge02LengthSq ? edge01[1] : edge02[1];
		axis[2] = edge01LengthSq >= edge02LengthSq ? edge01[2] : edge02[2];
		if (!Normalize3(axis))
		{
			return false;
		}
		if (ShouldFlipCanonicalAxis(axis))
		{
			axis[0] = -axis[0];
			axis[1] = -axis[1];
			axis[2] = -axis[2];
		...
		float widthAxis[3] = {};
		Cross3(normal, axis, widthAxis);
		if (!Normalize3(widthAxis))
		{
			return false;
		}

		LaserLineSegmentCandidate candidate = {};
		candidate.surfaceIndex = surfaceIndex;
		candidate.textureId = surface.material.texture->GetID().GetIndex();
		candidate.palette = surface.material.palette;
		candidate.shade = surface.material.shade;
		candidate.alpha = surface.material.alpha;
		candidate.flags = surface.material.flags;
		candidate.drawListType = surface.provenance.drawListType;
		Copy3(axis, candidate.axis);
		Copy3(normal, candidate.normal);
		Copy3(widthAxis, candidate.widthAxis);

		candidate.minT = 1.0e30f;
		candidate.maxT = -1.0e30f;
		for (const CapturedVertex& vertex : surface.vertices)
		{
			const float t = Dot3(vertex.position, candidate.axis);
			candidate.minT = (std::min)(candidate.minT, t);
			candidate.maxT = (std::max)(candidate.maxT, t);
			candidate.center[0] += vertex.position[0];
			candidate.center[1] += vertex.position[1];
			candidate.center[2] += vertex.position[2];
			candidate.prevDelta[0] += vertex.prevPosition[0] - vertex.position[0];
			candidate.prevDelta[1] += vertex.prevPosition[1] - vertex.position[1];
			candidate.prevDelta[2] += vertex.prevPosition[2] - vertex.position[2];
		}

		for (int i = 0; i < 3; ++i)
		{
			candidate.center[i] *= 0.25f;
			candidate.prevDelta[i] *= 0.25f;
		}

		const float centerT = Dot3(candidate.center, candidate.axis);
		AddScaled3(candidate.center, candidate.axis, -centerT, candidate.perpendicularCenter);

		for (const CapturedVertex& vertex : surface.vertices)
		{
			float fromCenter[3] = {};
			Sub3(vertex.position, candidate.center, fromCenter);
			candidate.halfWidth = (std::max)(candidate.halfWidth, fabsf(Dot3(fromCenter, candidate.widthAxis)));
		}

		if (candidate.maxT - candidate.minT <= candidate.halfWidth * 2.0f || candidate.halfWidth <= 0.001f)
		{
			return false;
		}
	...
	bool LaserLineCandidatesShareLine(const LaserLineSegmentCandidate& a, const LaserLineSegmentCandidate& b)
	{
		if (!SameLaserLineMaterial(a, b) ||
			Dot3(a.axis, b.axis) < kLaserLineAxisDotTolerance ||
			Dot3(a.normal, b.normal) < kLaserLineNormalDotTolerance ||
			Dot3(a.widthAxis, b.widthAxis) < kLaserLineNormalDotTolerance)
		{
			return false;
		}

		float perpendicularDelta[3] = {};
		Sub3(b.perpendicularCenter, a.perpendicularCenter, perpendicularDelta);
		if (LengthSq3(perpendicularDelta) > kLaserLinePerpendicularTolerance * kLaserLinePerpendicularTolerance)
		{
			return false;
		}

		const float widthTolerance = (std::max)(0.25f, a.halfWidth * 0.25f);
		return fabsf(a.halfWidth - b.halfWidth) <= widthTolerance;
	}

	SurfaceRef BuildMergedLaserLineSurface(const std::vector<SurfaceRef>& surfaces, const std::vector<const LaserLineSegmentCandidate*>& run)
	{
		const LaserLineSegmentCandidate& seed = *run.front();
		SurfaceRef merged = surfaces[seed.surfaceIndex];
		...
		float prevDelta[3] = {};
		float halfWidth = 0.0f;
		for (const LaserLineSegmentCandidate* candidate : run)
		{
			minT = (std::min)(minT, candidate->minT);
			maxT = (std::max)(maxT, candidate->maxT);
			halfWidth += candidate->halfWidth;
			for (int i = 0; i < 3; ++i)
			{
				perpendicularCenter[i] += candidate->perpendicularCenter[i];
				prevDelta[i] += candidate->prevDelta[i];
			}
		}

		const float invCount = 1.0f / (float)run.size();
		halfWidth *= invCount;
		for (int i = 0; i < 3; ++i)
		{
			perpendicularCenter[i] *= invCount;
			prevDelta[i] *= invCount;
		}

		float startCenter[3] = {};
		float endCenter[3] = {};
		AddScaled3(perpendicularCenter, seed.axis, minT, startCenter);
		AddScaled3(perpendicularCenter, seed.axis, maxT, endCenter);

		auto makeVertex = [&](const float* center, float widthSign, float u, float v)
		{
			CapturedVertex vertex = {};
			AddScaled3(center, seed.widthAxis, widthSign * halfWidth, vertex.position);
			vertex.prevPosition[0] = vertex.position[0] + prevDelta[0];
			vertex.prevPosition[1] = vertex.position[1] + prevDelta[1];
			vertex.prevPosition[2] = vertex.position[2] + prevDelta[2];
			vertex.uv[0] = u;
			vertex.uv[1] = v;
			return vertex;
		};

		const float mergedLength = (std::max)(1.0f, maxT - minT);
		const float uMax = mergedLength / 64.0f;
		merged.vertices.reserve(4);
		merged.vertices.push_back(makeVertex(startCenter, -1.0f, 0.0f, 0.0f));
		merged.vertices.push_back(makeVertex(endCenter, -1.0f, uMax, 0.0f));
		merged.vertices.push_back(makeVertex(startCenter, 1.0f, 0.0f, 1.0f));
		merged.vertices.push_back(makeVertex(endCenter, 1.0f, uMax, 1.0f));
		merged.indices = { 0u, 1u, 2u, 2u, 1u, 3u };
		return merged;
	}

	void AggregateDukeLaserLineWalls(std::vector<SurfaceRef>& walls)
	{
		if (walls.size() < 2)
		{
			return;
		...
		std::vector<LaserLineSegmentCandidate> candidates;
		std::vector<int> candidateForSurface(walls.size(), -1);
		candidates.reserve(walls.size());
		for (size_t i = 0; i < walls.size(); ++i)
		{
			LaserLineSegmentCandidate candidate = {};
			if (TryBuildLaserLineCandidate(walls[i], i, candidate))
			{
				candidateForSurface[i] = (int)candidates.size();
		...
		std::vector<bool> consumed(walls.size(), false);
		std::vector<SurfaceRef> aggregated;
		aggregated.reserve(walls.size());

		for (size_t surfaceIndex = 0; surfaceIndex < walls.size(); ++surfaceIndex)
		{
			const int seedCandidateIndex = candidateForSurface[surfaceIndex];
			if (seedCandidateIndex < 0)
			{
				aggregated.push_back(std::move(walls[surfaceIndex]));
				continue;
			}
			if (consumed[surfaceIndex])
			{
				continue;
			}

			const LaserLineSegmentCandidate& seed = candidates[(size_t)seedCandidateIndex];
			std::vector<const LaserLineSegmentCandidate*> lineCandidates;
			lineCandidates.reserve(candidates.size());
			for (const LaserLineSegmentCandidate& candidate : candidates)
			{
				if (!consumed[candidate.surfaceIndex] && LaserLineCandidatesShareLine(seed, candidate))
				{
					lineCandidates.push_back(&candidate);
				}
			}

			std::sort(lineCandidates.begin(), lineCandidates.end(), [](const LaserLineSegmentCandidate* a, const LaserLineSegmentCandidate* b)
			{
				return a->minT < b->minT;
			});

			std::vector<const LaserLineSegmentCandidate*> run;
			for (size_t start = 0; start < lineCandidates.size();)
			{
				size_t end = start + 1;
				float currentMaxT = lineCandidates[start]->maxT;
				while (end < lineCandidates.size() && lineCandidates[end]->minT <= currentMaxT + kLaserLineGapTolerance)
				{
					currentMaxT = (std::max)(currentMaxT, lineCandidates[end]->maxT);
					++end;
				}
				...
				{
					if (lineCandidates[i]->surfaceIndex == surfaceIndex)
					{
						containsSeed = true;
						break;
					}
				...
				if (containsSeed)
				{
					run.assign(lineCandidates.begin() + start, lineCandidates.begin() + end);
					break;
				}
			...
			for (const LaserLineSegmentCandidate* candidate : run)
			{
				consumed[candidate->surfaceIndex] = true;
			}
			aggregated.push_back(BuildMergedLaserLineSurface(walls, run));
		}

		walls = std::move(aggregated);
	}
...
bool CaptureDynamicScene(HWDrawInfo& di, SceneView& outView, DynamicVoxelCaptureMode voxelCaptureMode)
{
...

	{
		...
		AggregateDukeLaserLineWalls(outView.opaqueWalls);
	}
	{
