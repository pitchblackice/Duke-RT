...
bool NRIRenderDevice::RenderPathTracedScene(HWDrawInfo& di, int drawmode, bool portal)
{
	...
	if (rendered && PerfLoopTraceActive())
	{
		...
		Printf(
			"PERF pt scene light caches NRI: frame=%llu static_probes=%u static_hits=%u static_material_hits=%u static_rebuilds=%u static_legacy=%u static_quarantined_fallbacks=%u static_validation_checks=%u static_validation_mismatches=%u static_quarantined_entries=%u emissive_probes=%u emissive_hits=%u emissive_rebuilds=%u emissive_live_fallbacks=%u emissive_quarantined_fallbacks=%u emissive_evaluated_records=%u emissive_reused_candidates=%u emissive_validation_checks=%u emissive_validation_mismatches=%u emissive_resident_entries=%u emissive_resident_results=%u emissive_quarantined_entries=%u\n",
			(unsigned long long)mLastFrameBoundaryStats.frameNumber,
			shell.sceneLightStaticCacheProbes,
			shell.sceneLightStaticCacheStableHits,
			shell.sceneLightStaticCacheMaterialHits,
			shell.sceneLightStaticCacheRebuilds,
			shell.sceneLightStaticCacheLegacyFallbacks,
			shell.sceneLightStaticCacheQuarantinedFallbacks,
			shell.sceneLightStaticCacheValidationChecks,
			shell.sceneLightStaticCacheValidationMismatches,
			shell.sceneLightStaticCacheQuarantinedEntries,
			shell.sceneLightEmissiveCacheProbes,
			shell.sceneLightEmissiveCacheHits,
			shell.sceneLightEmissiveCacheRebuilds,
			shell.sceneLightEmissiveCacheLiveFallbacks,
			shell.sceneLightEmissiveCacheQuarantinedFallbacks,
			shell.sceneLightEmissiveCacheEvaluatedRecords,
			shell.sceneLightEmissiveCacheReusedCandidates,
			shell.sceneLightEmissiveCacheValidationChecks,
			shell.sceneLightEmissiveCacheValidationMismatches,
			shell.sceneLightEmissiveCacheResidentEntries,
			shell.sceneLightEmissiveCacheResidentCandidates,
			shell.sceneLightEmissiveCacheQuarantinedEntries);
		Printf(
