...
bool SmokeEvaluateWorldEmissiveIncidentRis(
	SmokeEmissiveReservoirRecord record,
	float3 receiverPosition,
	bool diagnostics,
	uint requestedPointCandidates,
	out float3 incidentRadiance,
	out float3 lightDirection,
	out float distanceToLight,
	...
	out uint risRejects)
{
	...
	zeroProposals = 0u;
	risRejects = 0u;
	const uint pointCandidateCount = clamp(requestedPointCandidates, 1u, 8u);
	float targetSum = 0.0;
	float selectedTarget = 0.0;
	float3 selectedIncident = 0.0;
	float3 selectedDirection = 0.0;
	float selectedDistance = 0.0;
	uint selectionState = SmokeHash(record.SampleSeed ^ 0x68bc21ebu);
	[loop]
	for (uint pointIndex = 0u; pointIndex < pointCandidateCount; ++pointIndex)
	{
		SmokeEmissiveReservoirRecord pointRecord = record;
		// Candidate zero preserves the exact legacy point sequence, making K=1
		// a strict A/B fallback. Later candidates use independent dimensions.
		if (pointIndex != 0u)
			pointRecord.SampleSeed = SmokeHash(record.SampleSeed ^ SmokeHash(pointIndex + 0x02e5be93u));
		float3 pointIncident, pointDirection;
		float pointDistance;
		pointProposals++;
		if (!SmokeEvaluateWorldEmissiveIncident(pointRecord, receiverPosition, diagnostics,
			pointIncident, pointDirection, pointDistance))
		{
			zeroProposals++;
			continue;
		}
		const float pointTarget = SmokeEmissiveLuminance(pointIncident);
		if (!isfinite(pointTarget) || pointTarget <= 1e-8)
		{
			zeroProposals++;
			continue;
		}
		...
		if (SmokeRandom01(selectionState) * nextTargetSum < pointTarget)
		{
			...
			selectedIncident = pointIncident;
	...
	if (targetSum <= 1e-8 || selectedTarget <= 1e-8)
		return false;
	// Each point incident already contains the inverse conditional point PDF
	// through effectiveArea. This correction accounts only for RIS selection;
	// the outer local/global candidate PDF is applied once by the seed pass.
	const float risNormalization = targetSum / ((float)pointCandidateCount * selectedTarget);
	if (!isfinite(risNormalization) || risNormalization <= 0.0)
	{
		risRejects++;
		return false;
	}
	incidentRadiance = selectedIncident * risNormalization;
	lightDirection = selectedDirection;
	distanceToLight = selectedDistance;
	// Do not reapply the raw point's target threshold after the 1/K RIS
	// correction. A small but finite corrected estimate remains real energy.
