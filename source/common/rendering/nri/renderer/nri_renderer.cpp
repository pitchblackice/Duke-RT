...
namespace
{
	...
	static uint64_t HashResidentGeometryVertexPayload(uint64_t hash, const nri_scene::SceneVertex& vertex)
	{
		...
		{
			hash = CoherencyHashCombine64(hash, (uint64_t)CoherencyFloatBits(component));
		}
		...
		uint32_t primitiveIndex)
	{
		...
		const uint32_t visibilityChunk =
			...
			primitive.reserved0;
		hash = CoherencyHashCombine64(hash, (uint64_t)visibilityChunk);
		if (hasProvenance)
		{
			const auto& provenance = geometry.primitiveProvenance[primitiveIndex];
			hash = CoherencyHashCombine64(hash, (uint64_t)(uint32_t)provenance.sourceType);
			hash = CoherencyHashCombine64(hash, (uint64_t)(uint32_t)provenance.sectorIndex);
			hash = CoherencyHashCombine64(hash, (uint64_t)(uint32_t)provenance.wallIndex);
			hash = CoherencyHashCombine64(hash, (uint64_t)(uint32_t)provenance.sectionIndex);
			hash = CoherencyHashCombine64(hash, (uint64_t)(uint32_t)provenance.mapChunkIndex);
			hash = CoherencyHashCombine64(hash, (uint64_t)(uint32_t)provenance.nextSectorIndex);
			hash = CoherencyHashCombine64(hash, (uint64_t)(uint32_t)provenance.actorIndex);
			hash = CoherencyHashCombine64(hash, (uint64_t)provenance.drawListType);
			hash = CoherencyHashCombine64(hash, (uint64_t)provenance.cstat);
			hash = CoherencyHashCombine64(hash, (uint64_t)provenance.materialFlags);
		}
		...
		{
			hash = CoherencyHashCombine64(hash, UINT64_MAX);
		}
		return hash;
	}
		...
		uint32_t materialCount)
	{
		...
		uint64_t hash = 1469598103934665603ull;
		hash = CoherencyHashCombine64(hash, (uint64_t)vertexCount);
		hash = CoherencyHashCombine64(hash, (uint64_t)indexCount);
		hash = CoherencyHashCombine64(hash, (uint64_t)primitiveCount);
		hash = CoherencyHashCombine64(hash, (uint64_t)materialCount);
		...
		for (uint32_t i = 0; i < primitiveCount; ++i)
		{
			...
			const auto& primitive = geometry.primitives[primitiveIndex];
			hash = CoherencyHashCombine64(hash, (uint64_t)NormalizeResidentAtlasIndex(primitive.indices[0], vertexOffset));
			hash = CoherencyHashCombine64(hash, (uint64_t)NormalizeResidentAtlasIndex(primitive.indices[1], vertexOffset));
			hash = CoherencyHashCombine64(hash, (uint64_t)NormalizeResidentAtlasIndex(primitive.indices[2], vertexOffset));
			hash = CoherencyHashCombine64(hash, (uint64_t)NormalizeResidentAtlasIndex(primitive.materialIndex, materialOffset));
			...
			hash = CoherencyHashCombine64(hash, (uint64_t)primitive.flags);
			hash = CoherencyHashCombine64(hash, (uint64_t)primitive.portalIndex);
		...
		uint32_t materialCount)
	{
		...
		for (uint32_t i = 0; i < primitiveCount; ++i)
		{
			...
			primitiveHash = CoherencyHashCombine64(primitiveHash, (uint64_t)NormalizeResidentAtlasIndex(primitive.materialIndex, materialOffset));
			for (float component : primitive.normal)
			{
				primitiveHash = CoherencyHashCombine64(primitiveHash, (uint64_t)CoherencyFloatBits(component));
			}
			primitiveHash = CoherencyHashCombine64(primitiveHash, (uint64_t)primitive.flags);
			primitiveHash = CoherencyHashCombine64(primitiveHash, (uint64_t)primitive.portalIndex);
			...
			for (uint32_t corner = 0; corner < 3; ++corner)
			{
				...
				{
					primitiveHash = CoherencyHashCombine64(primitiveHash, (uint64_t)CoherencyFloatBits(primitiveUvs[corner][component]));
				}
			}
		...
		{
			hash = CoherencyHashCombine64(hash, primitiveHash);
		}
		return hash != 0 ? hash : 1;
	}
...
void NRIRenderer::OnLevelUnloadComplete(const LevelTransitionInfo& info)
{
	...
	assert(mRuntimeMutation.cache.chunks.empty());
...
NRIRenderer::LevelTransitionSnapshot NRIRenderer::BuildLevelTransitionSnapshot() const
{
	...
	snapshot.runtimeMutationChunkCount = (uint32_t)mRuntimeMutation.cache.chunks.size();
		...
		if (hasPersistentVoxelOverlay || hasRuntimeSpaceLinkOverlay || hasRuntimeMutationOverlay || hasActiveDynamicOverlay || hasMirrorExtendedDynamicOverlay || hasMirrorPlayerOverlay || hasRuntimeDebugSphereOverlay || hasSurfaceLightOverlay)
		{
			...
			if (paletteReady && texturesReady && buffersReady && accelerationReady)
			{
...

				{
					...
					{
						if (!hasRuntimeMutationOverlay && !mRuntimeMutation.lastFrame.active)
						{
							return 0ull;
						}
						uint64_t hash = 1469598103934665603ull;
						hash = HashCombine64(hash, (uint64_t)mRuntimeMutation.lastFrame.dirtyChunkCount);
						hash = HashCombine64(hash, (uint64_t)mRuntimeMutation.lastFrame.residentAppliedChunkCount);
						hash = HashCombine64(hash, (uint64_t)mRuntimeMutation.lastFrame.residentGeometryChunkCount);
						hash = HashCombine64(hash, (uint64_t)mRuntimeMutation.lastFrame.residentMaterialChunkCount);
						hash = HashCombine64(hash, (uint64_t)mRuntimeMutation.lastFrame.rebuiltChunkCount);
						hash = HashCombine64(hash, (uint64_t)mRuntimeMutation.lastFrame.heldChunkCount);
						hash = HashCombine64(hash, (uint64_t)mRuntimeMutation.lastFrame.replacementTriangleCount);
						hash = HashCombine64(hash, (uint64_t)mRuntimeMutation.lastFrame.materialCount);
						return hash;
					}();
...
void NRIRenderer::UploadChunkIndexDataToAtlas(
	const nri_scene::GeometryData& sourceGeometry,
	const StaticMapSceneCache::ChunkCache& sourceChunk,
	const StaticMapChunkAtlas::ChunkEntry& atlasChunk,
...
void NRIRenderer::SyncResidentMapChunkRegistryFromStaticScene()
{
	...
	for (size_t chunkListIndex = 0; chunkListIndex < mMapWorld.chunks.size(); ++chunkListIndex)
	{
		...
		if (chunkListIndex < mRuntimeMutation.cache.chunks.size())
		{
			const auto& replacement = mRuntimeMutation.cache.chunks[chunkListIndex];
...
void NRIRenderer::UpdateSurfaceProbe(const nri_scene::GeometryData& geometry, const nri_scene::MaterialBridgeData* materials, bool allowLogging)
{
	...
	if (result.provenance.mapChunkIndex >= 0)
	{
		...
		if (chunkIndex < mRuntimeMutation.cache.chunks.size())
		{
			const auto& replacement = mRuntimeMutation.cache.chunks[chunkIndex];
...
void NRIRenderer::PrintMapChunkDump(int32_t chunkIndex) const
{
	...
	const auto* replacement =
		(unsigned)chunkIndex < mRuntimeMutation.cache.chunks.size() ?
		&mRuntimeMutation.cache.chunks[(unsigned)chunkIndex] :
		nullptr;
...
void NRIRenderer::PrintMapChunkCompare(int32_t chunkIndex) const
{
	...
	for (auto& match : matches)
	{
			...
			staticSurface.kind != nri_scene::PTMapSurfaceKind::Portal)
		{
			...
			const bool adjacentReplaced =
				...
				adjacentChunkIt->second < mRuntimeMutation.cache.chunks.size() &&
				mRuntimeMutation.cache.chunks[adjacentChunkIt->second].active;
			if (adjacentReplaced)
			{
	...
	for (const auto& match : matches)
	{
		...
		const bool adjacentReplaced =
			...
			(unsigned)adjacentChunkIndex < mRuntimeMutation.cache.chunks.size() &&
			mRuntimeMutation.cache.chunks[(unsigned)adjacentChunkIndex].active;
		...
		const size_t chunkCount = std::min(mStaticMapScene.lightChunkViews.size(), mStaticMapScene.chunks.size());
		{
			...
			for (size_t chunkListIndex = 0; chunkListIndex < chunkCount; ++chunkListIndex)
			{
				...
				const bool useRuntimeMutationReplacement =
					mapChunkIndex < mRuntimeMutation.cache.chunks.size() &&
					mRuntimeMutation.cache.chunks[mapChunkIndex].active &&
					mRuntimeMutation.cache.chunks[mapChunkIndex].valid;
