...
NRIPassDispatchContext BuildNRIPassDispatchContext(NRIRenderer& renderer)
{
	...
	init.textures.user = &renderer;
	init.textures.getFrameTexture = [](void* user, NRIRenderer::FrameTextureSlot slot) -> NRITextureResource&
	{
		return static_cast<NRIRenderer*>(user)->GetFrameTexture(slot);
	};
	init.pipelines.user = &renderer;
	init.pipelines.getPipeline = [](void* user, NRIRenderer::PipelineSlot slot) -> nri::Pipeline*
	{
		return static_cast<NRIRenderer*>(user)->GetPipeline(slot);
	};
	init.descriptors.user = &renderer;
	init.descriptors.updateFrameTextureSet = [](void* user) -> bool
	{
		return NRIDescriptorSetManager::UpdateFrameTextureSet(*static_cast<NRIRenderer*>(user));
	};
	init.descriptors.updateFrameTextureSetWithDescriptors = [](void* user, nri::DescriptorSet* descriptorSet, const std::array<nri::Descriptor*, 14>& descriptors) -> bool
	{
		return NRIDescriptorSetManager::UpdateFrameTextureSet(*static_cast<NRIRenderer*>(user), descriptorSet, descriptors);
	};
	init.descriptors.updateOutputSet = [](void* user) -> bool
	{
		return NRIDescriptorSetManager::UpdateOutputSet(*static_cast<NRIRenderer*>(user));
	};
	init.descriptors.updateOutputSetWithDescriptors = [](void* user, nri::DescriptorSet* descriptorSet, const std::array<nri::Descriptor*, 15>& descriptors) -> bool
	{
		return NRIDescriptorSetManager::UpdateOutputSet(*static_cast<NRIRenderer*>(user), descriptorSet, descriptors);
	};
	init.resources.user = &renderer;
	init.resources.frameBuffer = renderer.mFrameBuffer;
	init.resources.core = renderer.mFrameBuffer != nullptr ? &renderer.mFrameBuffer->mCore : nullptr;
	init.resources.device = renderer.mFrameBuffer != nullptr ? renderer.mFrameBuffer->mDevice : nullptr;
	init.resources.commandBuffer = renderer.mFrameBuffer != nullptr ? renderer.mFrameBuffer->mCommandBuffer : nullptr;
	init.resources.buildResourceServices = [](void* user) -> NRIResourceServices
	{
		return static_cast<NRIRenderer*>(user)->BuildResourceServices();
	};
	init.resources.updateReprojectionBuffer = [](void* user) -> bool
	{
		return NRISceneUploadManager::UpdateReprojectionBuffer(*static_cast<NRIRenderer*>(user), nullptr);
	};
	init.resources.getOutputPolicy = [](void* user) -> NRIPTOutputPolicy
	{
		NRIRenderer* renderer = static_cast<NRIRenderer*>(user);
		return renderer->mFrameBuffer->GetPathTracingOutputPolicy();
	};
	init.resources.copyFinalToActiveTarget = [](void* user)
	{
		static_cast<NRIRenderer*>(user)->CopyFinalToActiveTarget();
	};
	init.resources.copyTexture = [](void* user, NRITextureResource& source, NRITextureResource& destination)
	{
		static_cast<NRIRenderer*>(user)->CopyTexture(source, destination);
	};
	init.resources.transitionTexture = [](void* user, NRITextureResource& texture, nri::AccessLayoutStage after)
	{
		NRIRenderer* renderer = static_cast<NRIRenderer*>(user);
		renderer->mFrameBuffer->TransitionTexture(texture, after);
	};
	init.commands.core = renderer.mFrameBuffer != nullptr ? &renderer.mFrameBuffer->mCore : nullptr;
	init.commands.commandBuffer = renderer.mFrameBuffer != nullptr ? renderer.mFrameBuffer->mCommandBuffer : nullptr;
	init.sceneBinding.user = &renderer;
	init.sceneBinding.getCurrentSceneTextureSet = [](void* user) -> nri::DescriptorSet*
	{
		return static_cast<NRIRenderer*>(user)->GetCurrentSceneTextureSet();
	};
	init.sceneBinding.getCurrentSceneDataSet = [](void* user) -> nri::DescriptorSet*
	{
		return static_cast<NRIRenderer*>(user)->GetCurrentSceneDataSet();
	};
	init.sceneBinding.bindSceneRootDescriptors = [](void* user)
	{
		static_cast<NRIRenderer*>(user)->BindSceneRootDescriptors();
	};
	init.exposureService.user = &renderer;
	init.exposureService.readbackAutoExposureStats = [](void* user)
	{
		static_cast<NRIRenderer*>(user)->ReadbackAutoExposureStats();
	};
	init.exposureService.ensureAutoExposureResources = [](void* user, const NRIAutoExposureSettings& settings) -> bool
	{
		return static_cast<NRIRenderer*>(user)->EnsureAutoExposureResources(settings);
	};
	init.exposureService.requestAutoExposureReset = [](void* user, const char* reason)
	{
		static_cast<NRIRenderer*>(user)->RequestAutoExposureReset(reason);
	};
	init.exposureService.dispatchAutoExposure = [](void* user, NRIRenderer::FrameTextureSlot sourceSlot) -> bool
	{
		return static_cast<NRIRenderer*>(user)->DispatchAutoExposure(sourceSlot);
	};
	init.exposureService.resolveExposureRoute = [](void* user, NRIRenderer::FrameTextureSlot inputSlot, const NRIPTOutputPolicy& outputPolicy, NRIMainUpscalerKind mainKind, NRIPostSharpenKind postSharpenKind) -> NRIRenderer::ExposureRoute
	{
		return static_cast<NRIRenderer*>(user)->ResolveExposureRoute(inputSlot, outputPolicy, mainKind, postSharpenKind);
	};
	init.upscalerService.user = &renderer;
	init.upscalerService.resolveMainUpscalerKind = [](void* user, bool logFallback) -> NRIMainUpscalerKind
	{
		return static_cast<NRIRenderer*>(user)->ResolveMainUpscalerKind(logFallback);
	};
	init.upscalerService.resolvePostSharpenKind = [](void* user, bool logFallback) -> NRIPostSharpenKind
	{
		return static_cast<NRIRenderer*>(user)->ResolvePostSharpenKind(logFallback);
	};
	init.upscalerService.getSelectedUpscalerMode = [](void* user) -> nri::UpscalerMode
	{
		return static_cast<NRIRenderer*>(user)->GetSelectedUpscalerMode();
	};
	init.upscalerService.shouldRunAppTaaForFrameGraph = [](void* user, NRIMainUpscalerKind kind) -> bool
	{
		return static_cast<NRIRenderer*>(user)->ShouldRunAppTaaForFrameGraph(kind);
	};
	init.upscalerService.traceTemporalState = [](void* user, const char* stage, NRIMainUpscalerKind resolvedMainUpscaler, NRIPostSharpenKind resolvedPostSharpen, bool runAppTaa, NRIRenderer::FrameTextureSlot primarySlot, NRIRenderer::FrameTextureSlot secondarySlot)
	{
	...
	init.upscalerService.ensureMainUpscaler = [](void* user, NRIMainUpscalerKind kind, nri::UpscalerMode mode, uint32_t outputWidth, uint32_t outputHeight, bool exposure) -> bool
	{
		NRIRenderer* renderer = static_cast<NRIRenderer*>(user);
		return renderer->mUpscaler.EnsureMainUpscaler(*renderer->mFrameBuffer, kind, mode, outputWidth, outputHeight, exposure);
	};
	init.upscalerService.dispatchMainUpscaler = [](void* user, NRIMainUpscalerKind kind, const NRIUpscalerDispatchDesc& desc) -> bool
	{
		NRIRenderer* renderer = static_cast<NRIRenderer*>(user);
		return renderer->mUpscaler.DispatchMainUpscaler(*renderer->mFrameBuffer, kind, desc);
	};
	init.upscalerService.ensurePostSharpen = [](void* user, NRIPostSharpenKind kind, uint32_t outputWidth, uint32_t outputHeight) -> bool
	{
		NRIRenderer* renderer = static_cast<NRIRenderer*>(user);
		return renderer->mUpscaler.EnsurePostSharpen(*renderer->mFrameBuffer, kind, outputWidth, outputHeight);
	};
	init.upscalerService.dispatchPostSharpen = [](void* user, NRIPostSharpenKind kind, const NRIUpscalerDispatchDesc& desc) -> bool
	{
		NRIRenderer* renderer = static_cast<NRIRenderer*>(user);
		return renderer->mUpscaler.DispatchPostSharpen(*renderer->mFrameBuffer, kind, desc);
	};
	init.selfTest.user = &renderer;
	init.selfTest.resetSelfTestRouteSnapshot = [](void* user)
	{
		static_cast<NRIRenderer*>(user)->ResetSelfTestRouteSnapshot();
	};
	init.selfTest.setSelfTestRouteSnapshot = [](void* user, const char* routeName, const char* presenterName, const char* ownerName, const char* passListName, bool denoiserRun, bool upscalerRun, bool exposureRun)
	{
	...
	init.pipelineLayout = &renderer.mPipelineLayout;
	init.taaPipelineLayout = &renderer.mTaaPipelineLayout;
	init.presentPipelineLayout = &renderer.mPresentPipelineLayout;
	init.samplerSet = &renderer.mSamplerSet;
	init.frameTextureSet = &renderer.mFrameTextureSet;
	init.outputSet = &renderer.mOutputSet;
	init.compositionFrameTextureSet = &renderer.mCompositionFrameTextureSet;
	init.compositionOutputSet = &renderer.mCompositionOutputSet;
	init.upscalerPrepassFrameTextureSet = &renderer.mUpscalerPrepassFrameTextureSet;
	init.upscalerPrepassOutputSet = &renderer.mUpscalerPrepassOutputSet;
	init.taaFrameTextureSet = &renderer.mTaaFrameTextureSet;
	init.taaOutputSet = &renderer.mTaaOutputSet;
	init.rawPresentFrameTextureSet = &renderer.mRawPresentFrameTextureSet;
	init.rawPresentOutputSet = &renderer.mRawPresentOutputSet;
	init.finalPresentFrameTextureSet = &renderer.mFinalPresentFrameTextureSet;
	init.finalPresentOutputSet = &renderer.mFinalPresentOutputSet;
	init.frameInputDescriptors = &renderer.mFrameInputDescriptors;
	init.outputDescriptors = &renderer.mOutputDescriptors;
	init.exposure = &renderer.mExposure;
	init.traceShaderStats = &renderer.mTraceShaderStats;
	init.nrd = &renderer.mNrd;
	init.upscaler = &renderer.mUpscaler;
	init.persistentVoxels = &renderer.mPersistentVoxels;
	init.boundSceneInstances = &renderer.mBoundSceneInstances;
	init.sceneInstanceBuffer = &renderer.mSceneInstanceBuffer;
	init.directionalLightState = &renderer.mDirectionalLightState;
	init.nightVisionState = &renderer.mNightVisionState;
	init.lastPerfShellTraceStats = &renderer.mLastPerfShellTraceStats;
	init.lastPerfTraceShaderStats = &renderer.mLastPerfTraceShaderStats;
	init.lastAutoExposureSettings = &renderer.mLastAutoExposureSettings;
	init.frameIndex = &renderer.mFrameIndex;
	init.renderWidth = &renderer.mRenderWidth;
	init.renderHeight = &renderer.mRenderHeight;
	init.outputWidth = &renderer.mOutputWidth;
	init.outputHeight = &renderer.mOutputHeight;
	init.targetWidth = &renderer.mTargetWidth;
	init.targetHeight = &renderer.mTargetHeight;
	init.sceneLeft = &renderer.mSceneLeft;
	init.sceneTop = &renderer.mSceneTop;
	init.currentCameraPos = renderer.mCurrentCameraPos;
	init.currentCameraForward = renderer.mCurrentCameraForward;
	init.currentCameraRight = renderer.mCurrentCameraRight;
	init.currentCameraUp = renderer.mCurrentCameraUp;
	init.previousCameraPos = renderer.mPreviousCameraPos;
	init.previousCameraForward = renderer.mPreviousCameraForward;
	init.previousCameraRight = renderer.mPreviousCameraRight;
	init.previousCameraUp = renderer.mPreviousCameraUp;
	init.currentTanHalfFovX = &renderer.mCurrentTanHalfFovX;
	init.currentTanHalfFovY = &renderer.mCurrentTanHalfFovY;
	init.previousTanHalfFovX = &renderer.mPreviousTanHalfFovX;
	init.previousTanHalfFovY = &renderer.mPreviousTanHalfFovY;
	init.currentJitter = renderer.mCurrentJitter;
	init.previousJitter = renderer.mPreviousJitter;
	init.currentViewToClip = renderer.mCurrentViewToClip;
	init.previousViewToClip = renderer.mPreviousViewToClip;
	init.currentWorldToView = renderer.mCurrentWorldToView;
	init.previousWorldToView = renderer.mPreviousWorldToView;
	init.skyColor = renderer.mSkyColor;
	init.groundColor = renderer.mGroundColor;
	init.guiCaptureActive = &renderer.mGuiCaptureActive;
	init.resetHistory = &renderer.mResetHistory;
	init.hasAutoExposureSettingsState = &renderer.mHasAutoExposureSettingsState;
	init.useUpscaledInFinal = &renderer.mUseUpscaledInFinal;
	init.useDenoisedCompositionInputs = &renderer.mUseDenoisedCompositionInputs;
	init.useSplitShadowDenoiser = &renderer.mUseSplitShadowDenoiser;
	init.boundStaticPrimitiveCount = &renderer.mBoundStaticPrimitiveCount;
	init.boundDynamicPrimitiveCount = &renderer.mBoundDynamicPrimitiveCount;
	init.boundStaticMaterialCount = &renderer.mBoundStaticMaterialCount;
	init.boundDynamicMaterialCount = &renderer.mBoundDynamicMaterialCount;
	init.boundPortalCount = &renderer.mBoundPortalCount;
	init.boundRuntimeLightCount = &renderer.mBoundRuntimeLightCount;
	init.boundRuntimeLightTileCountX = &renderer.mBoundRuntimeLightTileCountX;
	init.boundRuntimeLightTileCountY = &renderer.mBoundRuntimeLightTileCountY;
	init.historyInputSlot = &renderer.mHistoryInputSlot;
	init.historyOutputSlot = &renderer.mHistoryOutputSlot;
	init.upscaledInputSlot = &renderer.mUpscaledInputSlot;
