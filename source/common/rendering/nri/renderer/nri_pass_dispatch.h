...
public:
	...
	const uint32_t& mRenderWidth;
	const uint32_t& mRenderHeight;
	const uint32_t& mOutputWidth;
	const uint32_t& mOutputHeight;
	const uint32_t& mTargetWidth;
	const uint32_t& mTargetHeight;
	const int32_t& mSceneLeft;
	const int32_t& mSceneTop;
	const float* mCurrentCameraPos = nullptr;
	const float* mCurrentCameraForward = nullptr;
	const float* mCurrentCameraRight = nullptr;
	const float* mCurrentCameraUp = nullptr;
	const float* mPreviousCameraPos = nullptr;
	const float* mPreviousCameraForward = nullptr;
	const float* mPreviousCameraRight = nullptr;
	const float* mPreviousCameraUp = nullptr;
	const float& mCurrentTanHalfFovX;
	const float& mCurrentTanHalfFovY;
	const float& mPreviousTanHalfFovX;
	const float& mPreviousTanHalfFovY;
	const float* mCurrentJitter = nullptr;
	const float* mPreviousJitter = nullptr;
	const float* mCurrentViewToClip = nullptr;
	const float* mPreviousViewToClip = nullptr;
	const float* mCurrentWorldToView = nullptr;
	const float* mPreviousWorldToView = nullptr;
	const float* mSkyColor = nullptr;
	const float* mGroundColor = nullptr;
	const bool& mGuiCaptureActive;
	const bool& mResetHistory;
	bool& mHasAutoExposureSettingsState;
	bool& mUseUpscaledInFinal;
	bool& mUseDenoisedCompositionInputs;
	bool& mUseSplitShadowDenoiser;
	const uint32_t& mSceneInstanceCount;
	const uint32_t& mBoundStaticPrimitiveCount;
	const uint32_t& mBoundDynamicPrimitiveCount;
	const uint32_t& mBoundStaticMaterialCount;
	const uint32_t& mBoundDynamicMaterialCount;
	const uint32_t& mBoundPortalCount;
	const uint32_t& mBoundRuntimeLightCount;
	const uint32_t& mBoundRuntimeLightTileCountX;
	const uint32_t& mBoundRuntimeLightTileCountY;
