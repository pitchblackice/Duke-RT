...
namespace
{
	...
	void FlushRecordOwners(const Record& record)
	{
		...
		Printf("PERF pt shell trace NRI: frame=%llu nri_frame=%llu total=%.3f init=%.3f map=%.3f state=%.3f select=%.3f lights=%.3f frame_graph=%.3f post_diag=%.3f unattributed=%.3f active_prims=%u dynamic_prims=%u materials=%u instances=%u compact=1 epoch=%llu sample=%u\n",
			...
			record.eligibleIndex);
		Printf("PERF pt scene select accounting NRI: frame=%llu nri_frame=%llu total=%.3f mutation=%.3f dynamic_capture=%.3f persistent_batch=%.3f material_bridge=%.3f textures=%.3f buffer_upload=%.3f persistent_voxel_as=%.3f dynamic_as=%.3f world_tlas=%.3f scene_data=%.3f state_commit=%.3f unaccounted=%.3f structural=%u material=%u resident=%u upload_bytes=%llu compact=1 epoch=%llu sample=%u\n",
			(unsigned long long)outer.traceFrame, (unsigned long long)nri.frame,
			nri.selectMs, nri.mutationMs, nri.dynamicCaptureMs,
