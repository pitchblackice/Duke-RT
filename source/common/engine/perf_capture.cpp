...
namespace
{
	...
	void FlushRecordOwners(const Record& record)
	{
		...
		Printf("PERF render trace NRI: frame=%llu total=%.3f init=%.3f res=%.3f state=%.3f capture=%.3f geo=%.3f mats=%.3f textures=%.3f buffers=%.3f as=%.3f graph=%.3f wait=%.3f wait_present=%.3f acquire=%.3f submit=%.3f present=%.3f trace=%.3f denoise=%.3f compose=%.3f upscale=%.3f final=%.3f compact=1 epoch=%llu sample=%u\n",
			(unsigned long long)outer.traceFrame, outer.nriTotalMs, outer.nriInitializeMs,
		...
		Printf("PERF pt shell trace NRI: frame=%llu total=%.3f init=%.3f map=%.3f state=%.3f select=%.3f lights=%.3f frame_graph=%.3f post_diag=%.3f unattributed=%.3f active_prims=%u dynamic_prims=%u materials=%u instances=%u compact=1 epoch=%llu sample=%u\n",
			(unsigned long long)nri.frame, nri.totalMs, nri.initMs, nri.mapMs, nri.stateMs,
			...
			record.eligibleIndex);
		Printf("PERF pt scene select accounting NRI: frame=%llu total=%.3f mutation=%.3f dynamic_capture=%.3f persistent_batch=%.3f material_bridge=%.3f textures=%.3f buffer_upload=%.3f persistent_voxel_as=%.3f dynamic_as=%.3f world_tlas=%.3f scene_data=%.3f state_commit=%.3f unaccounted=%.3f structural=%u material=%u resident=%u upload_bytes=%llu compact=1 epoch=%llu sample=%u\n",
			(unsigned long long)nri.frame, nri.selectMs, nri.mutationMs, nri.dynamicCaptureMs,
			...
			nri.mutationMaterial, nri.mutationResident,
			(unsigned long long)nri.sceneUploadBytes, (unsigned long long)gCapture.epoch,
			record.eligibleIndex);
		Printf("PERF pt resource waits NRI: frame=%llu total=%u/%.3f compact=1 epoch=%llu sample=%u\n",
			(unsigned long long)nri.frame, nri.resourceWaitCalls, nri.resourceWaitMs,
			(unsigned long long)gCapture.epoch, record.eligibleIndex);
		Printf("PERF pt gpu timing NRI: frame=%llu segment=%.3f scene=%.3f trace=%.3f denoise=%.3f compose=%.3f upscale=%.3f final=%.3f segments=%u invalid=%u dropped=%u resolved=%u expected=%u compact=1 epoch=%llu sample=%u\n",
			(unsigned long long)nri.frame, record.gpu.segmentMs, record.gpu.sceneMs,
