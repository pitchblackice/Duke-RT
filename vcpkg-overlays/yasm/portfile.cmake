...
# Patch LOCATION lookups to generator expressions since modern CMake forbids LOCATION.
set(YASM_MACROS "${SOURCE_PATH}/cmake/modules/YasmMacros.cmake")
file(READ "${YASM_MACROS}" _yasm_macros)
string(REPLACE "get_target_property(_tmp_GENPERF_EXE genperf LOCATION)"
               "set(_tmp_GENPERF_EXE \"$<TARGET_FILE:genperf>\")"
               _yasm_macros "${_yasm_macros}")
string(REPLACE "get_target_property(_tmp_RE2C_EXE re2c LOCATION)"
               "set(_tmp_RE2C_EXE \"$<TARGET_FILE:re2c>\")"
               _yasm_macros "${_yasm_macros}")
string(REPLACE "get_target_property(_tmp_GENMACRO_EXE genmacro LOCATION)"
               "set(_tmp_GENMACRO_EXE \"$<TARGET_FILE:genmacro>\")"
               _yasm_macros "${_yasm_macros}")
file(WRITE "${YASM_MACROS}" "${_yasm_macros}")

set(YASM_NASM_CMAKE "${SOURCE_PATH}/modules/preprocs/nasm/CMakeLists.txt")
file(READ "${YASM_NASM_CMAKE}" _yasm_nasm)
string(REPLACE "get_target_property(_tmp_GENVERSION_EXE genversion LOCATION)"
               "set(_tmp_GENVERSION_EXE \"$<TARGET_FILE:genversion>\")"
               _yasm_nasm "${_yasm_nasm}")
file(WRITE "${YASM_NASM_CMAKE}" "${_yasm_nasm}")
